...
export async function computeCollectorDNA(userId: string): Promise<CollectorDNA> {
  ...
  return {
    ...
    achievements: [], // populated by achievementService against real unlock thresholds
...
async function generateDNANarrative(input: {
  ...
  const completion = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: `You write the narrative layer of a collector's "Collector DNA" profile.
You are given REAL computed metrics — never invent numbers not provided. Ground every sentence in
the given data. Respond as JSON:
{
  "summary": string (2-3 sentences),
  "insights": [{ "text": string, "basedOn": string[] }] (3-5 items),
  "funFacts": string[] (2-4 items),
  "predictions": [{ "text": string, "confidence": number }] (2-3 items, speculative and clearly framed as such),
  "diversificationSuggestions": string[] (1-3 items),
  "compass": { "current": string, "projected": string, "explanation": string },
  "coach": { "strengths": string[], "weaknesses": string[], "opportunities": string[], "recommendations": string[], "weekOf": string }
}
Use these archetypes for "current"/"projected": INVESTOR, HISTORIAN, COMPLETIONIST, CURATOR, FLIPPER, EXPLORER, TREND_HUNTER, MINIMALIST, LUXURY_COLLECTOR, COMMUNITY_COLLECTOR, PRESERVATIONIST, COMPETITIVE_COLLECTOR.
